...
class BagDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        leading: IconButton(
          icon: const Icon(Icons.arrow_back_ios_new, size: 18, color: AppColors.lavender),
        ...
        title: Column(
          ...
          children: [
            ...
            Text('${medicine.englishName} · ${medicine.category}',
              style: const TextStyle(fontSize: 10, color: AppColors.textHint, fontWeight: FontWeight.w400)),
          ],
        ),
        toolbarHeight: 60,
      ),
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                Wrap(
                  ...
                  children: [
                    _InfoBadge(label: medicine.timesOfDay.map((t) => t.label).join('·')),
                    _InfoBadge(label: medicine.mealTiming.label),
                    _InfoBadge(label: '1일 ${medicine.dailyCount}회'),
                    _InfoBadge(label: medicine.durationDays == 0 ? '계속 복용' : '${medicine.durationDays}일 처방'),
                  ],
                ),
                if (medicine.memo.isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    child: Text('메모: ${medicine.memo}',
                      style: const TextStyle(fontSize: 11, color: AppColors.lavenderDark, height: 1.5)),
                  ),
                ],
              ],
            ),
          ...
          _SectionCard(
            ...
            child: medicine.cautions.isEmpty
                ? const Text('주의사항 정보가 없습니다.',
                    style: TextStyle(fontSize: 11, color: AppColors.textHint))
                : Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: medicine.cautions.map((c) => Padding(
...
class _InfoBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.lavenderBg, borderRadius: BorderRadius.circular(8)),
