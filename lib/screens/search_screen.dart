...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _loadMyMedications() async {
    ...
    try {
      ...
      final selectedBagId = _selectedBagId ?? (bags.isNotEmpty ? bags.first.id : null);
      if (mounted) {
        setState(() {
          _myMedications = meds;
          _bags = bags;
  ...
  Future<void> _refreshCompareWarnings() async {
    ...
    if (comparisonDrugs.length < 2) {
      if (mounted) setState(() { _compareWarnings = []; _isComparing = false; });
      return;
    }
  ...
  void _selectBag(String bagId) => setState(() => _selectedBagId = bagId);

  String _drugKey(DrugInfo drug) =>
      drug.displayCode.isNotEmpty ? drug.displayCode : drug.name;

  List<DrugInfo> get _myDrugs =>
      _myMedications.map((m) => m.drug).whereType<DrugInfo>().toList();

  List<DrugInfo> get _comparisonDrugs => [..._myDrugs, ..._extraCompareDrugs];

  BagData? get _selectedBag {
    for (final bag in _bags) { if (bag.id == _selectedBagId) return bag; }
    return null;
  }
  ...
  Widget _buildMyMedicationPanel() {
    if (_isLoadingMyMeds) return const LinearProgressIndicator(minHeight: 2);
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            children: _bags.map((bag) {
              ...
              return Padding(
                ...
                child: ChoiceChip(
                  ...
                  avatar: Container(width: 8, height: 8,
                  ...
                  side: BorderSide(color: selected ? AppColors.lavender : AppColors.cardBorder),
                  labelStyle: TextStyle(
                    fontSize: 10,
                    color: selected ? AppColors.lavenderDark : AppColors.textSecondary,
        ...
        if (meds.isEmpty)
          const Padding(padding: EdgeInsets.only(top: 6),
              child: Text('선택한 분류에 담긴 약이 없습니다.',
                  style: TextStyle(fontSize: 10, color: AppColors.textHint)))
        else ...[
          const SizedBox(height: 6),
          Wrap(spacing: 6, runSpacing: 6, children: meds.map((med) => Chip(
            label: Text(med.displayName, overflow: TextOverflow.ellipsis),
            backgroundColor: AppColors.lavenderBg,
            side: BorderSide(color: AppColors.lavender.withOpacity(0.45)),
            labelStyle: const TextStyle(fontSize: 10, color: AppColors.lavenderDark, fontWeight: FontWeight.w600),
          )).toList()),
        ],
      ]),
    );
  ...
  Widget _buildBody() {
    ...
    return ListView(
      ...
      children: [
        ...
        ..._results.map((drug) {
          final key = _drugKey(drug);
          final isInBag = _myDrugs.any((item) => _drugKey(item) == key);
...
class _DrugResultCard extends StatelessWidget {
  ...
  const _DrugResultCard({
    required this.drug, required this.isSelected, required this.isInSelectedBag,
    required this.onTap, required this.onCompareTap,
  });
      ...
      child: InkWell(
        ...
        child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(drug.name, maxLines: 2, overflow: TextOverflow.ellipsis,
                style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
            const SizedBox(height: 4),
            Text(drug.company.isEmpty ? '업체 정보 없음' : drug.company,
                maxLines: 1, overflow: TextOverflow.ellipsis,
...
class _MiniBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: AppColors.lavenderBg, borderRadius: BorderRadius.circular(7)),
      child: Text(label, style: const TextStyle(fontSize: 9, color: AppColors.lavenderDark)),
    );
  }
}
