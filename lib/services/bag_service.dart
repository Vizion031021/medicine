...
class BagService {
  ...
  static Future<String> _bagsKey() async =>
      '${_bagsKeyPrefix}_${await _userScope()}';
      ...
      try {
        ...
        if (decoded is List) {
          bags.addAll(
            decoded
                .whereType<Map>()
                .map((item) => BagData.fromJson(Map<String, dynamic>.from(item)))
  ...
  static Future<void> _saveBags(SharedPreferences prefs, List<BagData> bags) async {
    ...
    if (!bags.any((bag) => bag.id == _defaultBag.id)) normalized.add(_defaultBag);
    normalized.addAll(bags.where((bag) => bag.id.isNotEmpty && bag.name.isNotEmpty));
